...
class KrakenPublicClient:
    ...
    def get_ohlc(self, pair: str, interval: int) -> pd.DataFrame:
...
def run_bot(max_loops: Optional[int] = None) -> None:
    ...
    print(f"Pair: {config.PAIR}, interval={config.INTERVAL}m, starting_cash=${config.STARTING_CASH:,.2f}")
    ...
    while True:
        ...
        try:
            df = client.get_ohlc(config.PAIR, config.INTERVAL)
